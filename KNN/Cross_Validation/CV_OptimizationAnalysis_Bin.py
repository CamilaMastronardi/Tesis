...
def CV_scores(numero_vecinos: int, cm: bool = True, 
    ...
    for i in cv_iter:
        path = f'KNN/Cross_Validation/ResultadosBinario_2min/CV_{numero_vecinos}vecinos_1000DTW_{i}_weighted.csv'
    ...
    if cm:
        ...
        plt.savefig(f'KNN/Cross_Validation/Figuras_Cross_Val_binario/cv_fig{numero_vecinos}vecinos_unweighted.png')
        plt.show()
        plt.close()
    ...
    print(f'{numero_vecinos}, {recall_weighted}')
...
def scores_plot():
    ...
    ax.scatter(ks, score_w, s = 200 ,marker='*', color = 'darkturquoise', label = 'Binario ponderado')
    ax.plot(ks, score_u, linestyle = '--', color = 'darkblue')
    ax.scatter(ks, score_u, s =100, marker='o', color = 'darkblue', label = 'Binario no ponderado')
    ax.legend(loc = (0.55,0.45), fontsize = 22)
