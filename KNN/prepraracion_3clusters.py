...
def trainingData3Clusters(groups_of_dates: list[str], use_cache: bool) -> pd.DataFrame: 
        ...
        for n in range(1, orbitas): 
            ...
            for mpb, bs in zip(time_MPB, time_BS):
                if is_orbit_clasified(df_not_marked, mpb, bs, 3):
                    ...
                    pedacitos_SW = cut_orbits_in_10_min(SW)
                    pedacitos_magnetosheath = cut_orbits_in_10_min(magnetosheath)
                    pedacitos_ionosphere = cut_orbits_in_10_min(ionosphere)
...
def cut_orbits_in_10_min(data: pd.DataFrame) -> pd.DataFrame:
    lista_pedacitos = [data[x:x+600] for x in range(0, len(data), 600)]
    result = []
...
if __name__== '__main__' :
    ...
    data_KNN_completed = trainingData3Clusters(['Group1','Group2','Group3','Group4'], use_cache = True)
    end_time = time.time()
